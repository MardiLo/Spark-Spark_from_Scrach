...
def trainEvaluateModel(trainData, validationData, iterations, step, regParam, regType, miniBatchFraction):
    record = dict()
    startTime = time()
    ...
    record['step'] = step
    record['regParam'] = regParam
    record['regType'] = regType
    record['miniBatchFraction'] = miniBatchFraction
    record['AUC'] = AUC
...
#
#
# def evalParameter(trainData, validationData, evalparm, iterationsList, stepList, regParamList, regTypeList, miniBatchFractionList):
#     metrics = [trainEvaluateModel(trainData, validationData, iterations, step, regParam, regType, miniBatchFraction)
...
def evalAllParameter(trainData, validationData, iterationsList, stepList, regParamList, regTypeList, miniBatchFractionList):
    metrics = [trainEvaluateModel(trainData, validationData, iterations, step, regParam, regType, miniBatchFraction)
               for iterations in iterationsList
               for step in stepList
               for regParam in regParamList
...
if __name__ == "__main__":
    # Set Spark Config. (run in local by a cpu and name the job as "StumbleUponEvergreenClassificationByLogReg")
    # Make a SparkContext object, and Depreciate Spark Warning
    conf = SparkConf().setMaster("local").setAppName("StumbleUponEvergreenClassificationByLogReg")
    ...
    iterationsList = [5, 10, 20]
    stepList = [50]
    regParamList = [0.1, 0.5, 1.0]
    regTypeList = [None, 'l2']
    miniBatchFractionList = [0.5]

    bestModel = evalAllParameter(trainData, validationData, iterationsList, stepList, regParamList, regTypeList, miniBatchFractionList)
